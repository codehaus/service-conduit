...
public class RetrRequestHandler implements RequestHandler {
    ...
    /**
     * Retrieves a file from the file system and writes out to the client. Currently only supports passive connections.
     * This means <code>RETR</code> command should be preceded by a <code>PASV</code> command.
    ...
    private void transfer(DefaultFtpSession session, int passivePort, DataConnection dataConnection, String fileName) {
        ...
        try {
            ...
            outputStream = dataConnection.getOutputStream();
            ...
            inputStream = ftpLet.onDownload(fileName, session);
