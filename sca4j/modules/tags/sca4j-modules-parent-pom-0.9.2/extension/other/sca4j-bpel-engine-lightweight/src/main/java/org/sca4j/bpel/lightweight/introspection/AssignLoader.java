...
public class AssignLoader implements TypeLoader<AssignDefinition> {
    ...
    @Override
    public AssignDefinition load(XMLStreamReader reader, IntrospectionContext context) throws XMLStreamException {
            ...
            switch (reader.next()) {
                case START_ELEMENT:
                    ...
                    if (elementName.equals(Constants.COPY_ELEMENT)) {
                        CopyDefinition copyDefinition = copyLoader.load(reader, context);
                        assignDefinition.getCopies().add(copyDefinition);
                    } else {
