...
public class SCA4JITestMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        RuntimeLifecycleCoordinator<MavenEmbeddedRuntime, ScdlBootstrapper> coordinator = instantiate(RuntimeLifecycleCoordinator.class, coordinatorImpl, bootClassLoader);
        coordinator.setConfiguration(configuration);
        bootRuntime(coordinator);
        
        try {
            TestRunner runner = new TestRunner(testDomain, compositeNamespace, compositeName, testScdl, reportsDirectory, trimStackTrace, buildDirectory, getLog());
    ...
    private MavenEmbeddedRuntime createRuntime(ClassLoader bootClassLoader, ClassLoader hostClassLoader, Set<URL> moduleDependencies) {
        MonitorFactory monitorFactory = new MavenMonitorFactory(getLog(), "sca4j");
